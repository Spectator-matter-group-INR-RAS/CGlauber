...
cola::LorentzVector FermiMomentum::randRotate(double p) {
  ...
  return cola::LorentzVector{std::pow(gconst::nucleonAverMass * gconst::nucleonAverMass + p * p, .5),
                             p * sinTheta * std::cos(phiVal), p * sinTheta * std::sin(phiVal), p * cosTheta};
}
...
double MorriseyMomentum::momentumMagn(int A, int NPart) { return enParam * std::sqrt(NPart) * normal(engine); }
